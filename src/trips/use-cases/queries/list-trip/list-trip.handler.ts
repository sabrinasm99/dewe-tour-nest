...
export class ListTripHandlerImpl implements ListTripHandler {
  ...
  async execute() {
    const response = await this.client.query(
      'SELECT trips.id, trips.title, countries.name AS country, trips.quota, trips.booked_slots, trips.accomodation, trips.transportation, trips.eat, trips.days, trips.nights, trips.date, trips.price, trips.description, trips.image FROM trips INNER JOIN countries ON trips.country_id = countries.id',
    );
